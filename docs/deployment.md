...
```bash
echo "deploy ALL=(ALL) NOPASSWD: /bin/systemctl restart wikiddle, /bin/systemctl restart caddy, /bin/systemctl daemon-reload, /bin/systemctl is-active --quiet wikiddle" | sudo tee /etc/sudoers.d/deploy-wikiddle
```
